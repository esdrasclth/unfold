...
Section Install
  ...
  ; Aparecer en "Abrir con".
  ;
  ; Desde Windows 10 una aplicacion no puede quedarse con una extension por su
  ; cuenta: la eleccion final la hace el usuario y vive en UserChoice, que esta
  ; protegida. APP_ASSOCIATE deja escrito el manejador, pero sin esta clave
  ; Unfold ni siquiera salia en la lista de programas con los que abrir un .md.
  {{#each file_associations as |association| ~}}
    {{#each association.ext as |ext| ~}}
...
Function .onInstSuccess
  ; En modo silencioso no hay pagina final donde marcar el acceso directo, asi
  ; que se acepta por linea de ordenes. Lo usa el instalador con interfaz
  ; propia, que ejecuta este por debajo con /S.
  ${GetParameters} $R0
  ClearErrors
